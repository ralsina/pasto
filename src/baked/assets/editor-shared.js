    ...
    if (isAutoDetect) {
      ...
      if (autoDetectOption) {
        ...
        if (jar && data.language && oldText !== autoDetectOption.textContent) {
          // Re-highlight without changing content to preserve cursor position
          const editorElement = document.getElementById('editor');
          if (editorElement && typeof window.highlight === 'function') {
            const currentTheme = localStorage.getItem('syntaxTheme') || 'default-dark';
            window.highlight(editorElement, data.language, currentTheme);
          }
        }
      }
