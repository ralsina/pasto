...
docker run --platform linux/arm64 -ti --rm -v "$PWD":/app --user="$UID" pasto-builder /bin/sh -c "cd /app && rm -rf lib shard.lock && shards build --release --without-development --static -Dno_watching --link-flags '-lssh -lssl -lcrypto' pasto-ssh"
