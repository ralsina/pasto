...
docker run -ti --rm -v "$PWD":/app --user="$UID" pasto-builder /bin/sh -c "cd /app && rm -rf lib shard.lock && shards build --without-development --static -Dinotify --link-flags '-lssh -lssl -lcrypto' pasto-ssh"
