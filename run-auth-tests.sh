...
# Authenticated User E2E Test Runner
# This script runs only authenticated user Playwright tests
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          Authenticated User E2E Test Runner               ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
show_usage() {
    ...
    echo ""
    echo "EXAMPLES:"
    echo "  $0                           # Run all authenticated tests"
    echo "  $0 -h                       # Run tests with browser visible"
    echo "  $0 -b chromium              # Run tests only on Chromium"
    echo "  $0 tests/api-key-management.spec.ts  # Run specific test file"
...
if [ -n "$SPEC_FILE" ]; then
    ...
    print_status "Running all authenticated tests with 2 workers to avoid rate limiting"
...
if [ -n "$SPEC_FILE" ]; then
    ...
    PLAYWRIGHT_CMD="$PLAYWRIGHT_CMD tests/auth-debug-mode.spec.ts tests/api-key-management.spec.ts tests/authenticated-user-features.spec.ts"
fi

# Set custom report directory
export PLAYWRIGHT_HTML_REPORT="$REPORT_DIR/html-report"

print_status "Starting authenticated user test execution..."
...
# Start Pasto server for authenticated tests
print_status "Starting Pasto server for authenticated user tests..."
...
if ! kill -0 $PASTO_PID 2>/dev/null; then
    print_error "Failed to start Pasto server for authenticated tests"
...
print_success "Pasto server started for authenticated tests (PID: $PASTO_PID)"
...
# Kill the server
if [ -f "test-results/pasto-auth.pid" ]; then
    PASTO_PID=$(cat "test-results/pasto-auth.pid")
    if kill -0 $PASTO_PID 2>/dev/null; then
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
    print_success "All authenticated tests passed! (Duration: ${DURATION}s)"
else
    print_error "Some authenticated tests failed! (Duration: ${DURATION}s)"
fi
...
echo -e "${BLUE}Server logs: test-results/pasto-auth.log${NC}"
